...
while (-not $sqlReady -and $attempts -lt $maxAttempts) {
    $attempts++
    try {
        $healthCheck = docker exec sqlserver /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "yourStrongPassword123#" -Q "SELECT 1" 2>&1
...
if (-not $sqlReady) {
    ...
    Write-Host "  Verifica los logs: docker logs sqlserver" -ForegroundColor Yellow
}
...
$migratorStatus = docker ps -a --filter "name=distribuidora-db-migrator" --format "{{.Status}}" 2>$null
if ($migratorStatus -match "Exited \(0\)") {
    ...
    Write-Host "  Ver logs: docker logs distribuidora-db-migrator" -ForegroundColor Gray
}
...
$workerStatus = docker inspect distribuidora-worker --format='{{.State.Status}}' 2>$null
...
$containers = docker ps -a --filter "name=distribuidora-" --format "table {{.Names}}\t{{.Status}}" 2>$null
if ($containers) {
...
try {
    $tablesCheck = docker exec sqlserver /opt/mssql-tools/bin/sqlcmd -S localhost -U sa -P "yourStrongPassword123#" -d distribuidora_db -Q "SELECT name FROM sys.tables WHERE name IN ('Calificaciones', 'ProductoStats')" -h -1 2>&1
...
Write-Host "  Ver logs del API:      docker logs -f distribuidora-api" -ForegroundColor Gray
Write-Host "  Ver logs de SQL:       docker logs -f sqlserver" -ForegroundColor Gray
Write-Host "  Ver logs del Worker:   docker logs -f distribuidora-worker" -ForegroundColor Gray
Write-Host "  Ver logs de Migrator:  docker logs distribuidora-db-migrator" -ForegroundColor Gray
